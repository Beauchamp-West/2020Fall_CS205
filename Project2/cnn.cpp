...
void Image::max_pool2() {
    ...
    for (int i = 0; i < depth; ++i) {
        for (int j = 0; j < out_s/depth; ++j) {
            ...
            f1 = data[1 + i * length / depth + r * width + c];
            f2 = data[1 + i * length / depth + r * width + c + 1];
            f3 = data[1 + i * length / depth + (r + 1) * width + c];
            f4 = data[1 + i * length / depth + (r + 1) * width + c + 1];

            out[1 + i * length / depth / 4 + j] = max_4(f1, f2, f3, f4);
        }
    }
...
void Image::fc2(int out_dep, const float * weight, const float * bias) {
    auto * out = new float[out_dep]();

    for (int i = 0; i < out_dep; ++i) {
        for (int j = 0; j < length; ++j) {
            out[i] += data[1+j] * weight[i * length + j];
        }
